...
class CredentialsControllerTest {
	...
	void testFinduserFailure() throws Exception {
		...
		ResponseEntity<Message> resp2 = testRestTemplate.exchange("http://localhost:" + port + "/login/find",
	...
	void testupdatePasswordSuccess() throws Exception {
		...
		headers.set("Content-type", MediaType.APPLICATION_JSON_VALUE);
		HttpEntity<Credentials> req = new HttpEntity<>(credentials, headers);
		ResponseEntity<Message> resp2 = testRestTemplate.exchange("http://localhost:" + port + "/login/update",
	...
	void testupdatePasswordFailure() throws Exception {
		Credentials credentials = new Credentials("shreya", "4321");

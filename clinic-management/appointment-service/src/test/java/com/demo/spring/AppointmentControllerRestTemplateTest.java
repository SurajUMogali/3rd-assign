...
class AppointmentControllerRestTemplateTest {
	...
	void testAddAppointmentPatientFailure() throws Exception {
		...
		stubFor(get(urlEqualTo("/clinic/doctor/find/1"))
		...
		ResponseEntity<String> response = testRestTemplate.exchange("http://localhost:" + port + "/appointment/save",
	...
	void testAddAppointmentDoctorFailure() throws Exception {
		...
		Message message = new Message("Doctor data is not available");
		String messageJson = mapper.writeValueAsString(message);
		...
		stubFor(get(urlEqualTo("/clinic/doctor/find/2"))
				.willReturn(aResponse().withHeader("Content-Type", "application/json").withBody(doctorJson)));
	...
	void testAddAppointmentSuccess() throws Exception {
		...
		Message message = new Message("Appointment already exists");
		String messageJson = mapper.writeValueAsString(message);
